...
flowchart TD
    ...
    Start(("开始")) --> DataLoad["1. 加载与准备数据<br>Load & Prepare Data"]
    DataLoad --> SampleCreate["2. 创建样本序列<br>Create Samples<br>(Enc_X, Dec_X, Target_Y)"]
    SampleCreate --> SplitData["3. 划分训练集/验证集<br>Train/Test Split"]
    SplitData --> DataLoader["封装 DataLoader<br>(Batch Size)"]
    
    DataLoader --> InitModel["4. 初始化模型与优化器<br>Model, Loss(MSE), Adam"]
    
    %% 训练主循环
    subgraph TrainingProcess ["训练主循环 Epoch Loop"]
        direction TB
        EpochStart{"Epoch < Max Epochs?"}
        
        subgraph TrainBatch ["训练批次 (Training)"]
            T_LoadBatch["加载 Batch 数据"] --> T_ToDevice["数据上载至 GPU"]
            T_ToDevice --> T_ZeroGrad["清空梯度 Zero Grad"]
            T_ZeroGrad --> T_Forward["前向传播 Forward<br>(模型预测)"]
            T_Forward --> T_Loss["计算损失 Loss"]
            T_Loss --> T_Backward["反向传播 Backward<br>(计算梯度)"]
            T_Backward --> T_Step["更新参数 Optimizer Step"]
        end
        
        subgraph ValBatch ["验证批次 (Validation)"]
            V_Mode["model.eval & no_grad"] --> V_LoadBatch["加载 Val Batch"]
            V_LoadBatch --> V_Forward["前向传播 Forward"]
            V_Forward --> V_CalcLoss["统计验证 Loss"]
        end

        EpochStart -- "是" --> T_LoadBatch
        T_Step --> |"Batch 结束?"| V_Mode
        V_CalcLoss --> |"Batch 结束?"| PrintLog["打印当前 Epoch 损失"]
        PrintLog --> EpochStart
    end

    EpochStart -- "否" --> SaveModel["6. 保存模型参数<br>.pth 文件"]
    SaveModel --> End(("结束"))

    class DataLoad,SampleCreate,SplitData,DataLoader,InitModel,SaveModel io;
    class T_LoadBatch,T_ToDevice,T_ZeroGrad,T_Forward,T_Loss,T_Backward,T_Step process;
    class V_Mode,V_LoadBatch,V_Forward,V_CalcLoss process;
    class EpochStart decision;
```
