...
def pad_or_truncate_array(arr: np.ndarray, target_length: int) -> np.ndarray:
    """
    将一维数组填充或截断到指定的目标长度。
    - 如果数组长度小于 target_length，则在右侧用 np.nan 填充。
    - 如果数组长度大于 target_length，则截断至 target_length。
    """
    result = np.full(target_length, np.nan, dtype=np.float32)
    len_to_copy = min(len(arr), target_length)
    result[:len_to_copy] = arr[:len_to_copy]
    return result
    ...
    if pathlib.Path(root_path).is_dir():
        ...
        if target_path.exists():
            ...
            if ds is not None:
                ...
                # 2. 使用辅助函数进行填充或截断，确保长度一致
                u_vals_final = pad_or_truncate_array(u_vals, time_count)
                v_vals_final = pad_or_truncate_array(v_vals, time_count)

                # 检查并打印警告（可选，但推荐）
                if len(u_vals) < time_count or len(v_vals_final) < time_count:
                    print(f'  [!] 警告: {target_file_name} 数据长度不足，已用 NaN 填充。')
                ...
                df_dict[temp_u_column_name] = u_vals_final.reshape(-1)
                df_dict[temp_v_column_name] = v_vals_final.reshape(-1)
    return df_dict


if __name__ == '__main__':
    fub_station_path = r'./../../data_common/fubs.csv'
    read_nc_path: str = r'Z:/WIND/GRAPES/2024'
    out_put_path: str = r'E:\01DATA\ML\WIND_FUBS_NEW'
    # step1: 读取站位信息
    fubs: List[dict] = read_stations(fub_station_path)
    """
        eg:
    ...
    for fub in fubs:
        temp_code: str = fub['code']
        temp_lat: float = fub['lat']
        temp_lng: float = fub['lng']
        # 使用一个字典来收集所有列数据
        all_fub_data = {}
        data_rows = []
        ...
        for temp_time in time_list:
            ...
            all_fub_data.update(temp_dict)
            print(f'[-] 处理{temp_time.format("YYYY-MM-DD HH:mm")}完成!')
            # 3. (重要) 将时间信息也加入到这个单行字典中
            temp_dict['time'] = temp_time
            # 4. 将这一整行数据（字典）追加到列表中
        ...
        # 如果某次循环返回的数据缺少某个键，Pandas 会在该行对应位置自动填充 NaN，而不会报错。
        temp_df = pd.DataFrame(all_fub_data)
        out_put_filename: str = f'FUB_{temp_code}_2024_uv.csv'
        save_path: str = str(pathlib.Path(out_put_path) / out_put_filename)
        temp_df.to_csv(save_path)
        print('-------------')
        print(f'输出文件:{out_put_filename}成功!')
        pass
