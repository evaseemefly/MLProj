...
def main():
    path_wind_fubs: Path = Path(r'E:\01DATA\ML\WIND_FUBS_NEW')
    """从预报风场提取的浮标预报数据集"""
    path_realdata_fubs: Path = Path(r'E:\01DATA\ML\FUB_UV')
    """提取的浮标实况数据集"""
    mergedata_h5_path: Path = Path(r'E:\01DATA\ML\MERGEDATA_H5\FUB')
    ...
    for temp_file in path_wind_fubs.rglob('*.csv'):
        # 'FUB_MF01002_2024_uv.csv'
        # 取 0 -23 行数据
        temp_df: pd.DataFrame = pd.read_csv(temp_file, nrows=24, index_col=0)
        """风场的 24 个预报时间数据(3hour)"""
        temp_code: str = temp_file.name.split('.')[0].split('_')[1]
        """当前遍历的 fub code"""

        temp_code_h5_filename: str = f'2024_{temp_code}_mergedata.h5'

        temp_code_h5_fullpath: Path = mergedata_h5_path / temp_code_h5_filename

        # 获取 dataframe 中的所有时间戳
        """
            eg:
                ['2024121212', '2024071700', '2024082712', ]
        """
        temp_forecast_timestamps = list(set([temp_col.split('_')[0] for temp_col in temp_df.columns[0:]]))
        # 排序
        ...
        for temp_time in temp_forecast_timestamps:
            ...
            # 20240101000000_u
            # 202401000000_u
            temp_time: arrow.Arrow = arrow.get(temp_time, 'YYYYMMDDHH')
            temp_time_str: str = temp_time.format('YYYYMMDDHH')
            # 2024010112_u
            # 提取并按照时间拼接 浮标的预报 u | v 数据
            ...
            """
                DatetimeIndex(['2024-01-01 00:00:00+00:00', '2024-01-01 03:00:00+00:00',
               '2024-01-01 06:00:00+00:00', '2024-01-01 09:00:00+00:00',
               '2024-01-01 12:00:00+00:00', '2024-01-01 15:00:00+00:00',
               '2024-01-01 18:00:00+00:00', '2024-01-01 21:00:00+00:00',
               '2024-01-02 00:00:00+00:00', '2024-01-02 03:00:00+00:00',
               '2024-01-02 06:00:00+00:00', '2024-01-02 09:00:00+00:00',
               '2024-01-02 12:00:00+00:00', '2024-01-02 15:00:00+00:00',
               '2024-01-02 18:00:00+00:00', '2024-01-02 21:00:00+00:00',
               '2024-01-03 00:00:00+00:00', '2024-01-03 03:00:00+00:00',
               '2024-01-03 06:00:00+00:00', '2024-01-03 09:00:00+00:00',
               '2024-01-03 12:00:00+00:00', '2024-01-03 15:00:00+00:00',
               '2024-01-03 18:00:00+00:00', '2024-01-03 21:00:00+00:00'],
            ...
            temp_station_realdata_full_path: Path = path_realdata_fubs / temp_station_realdata_name
            if temp_station_realdata_full_path.exists():
                try:
                    ...
                    temp_df_realdata.rename(columns={'u': 'realdata_u', 'v': 'realdata_v'},
                                            inplace=True)
                    # 转换 dataframe 的 index ,注意此种方式转换后的 index 不带时区，需要手动添加时区为世界时
                    """
                        DatetimeIndex(['2024-01-01 00:00:00', '2024-01-01 01:00:00',
                                      ...
                                      dtype='datetime64[ns]', name='DT', length=8784, freq=None)
                    """
                    temp_df_realdata.index = pd.to_datetime(temp_df_realdata.index)
                    # 将 temp_df_realdata 的索引原地更新为带时区的版本
                    temp_df_realdata.index = temp_df_realdata.index.tz_localize('UTC')
                    # 为不带时区的索引本地化为 UTC 时区
                    # temp_df_realdata_aware_index = temp_df_realdata.index.tz_localize('UTC')
